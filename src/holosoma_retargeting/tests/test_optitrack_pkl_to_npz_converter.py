...
from holosoma_retargeting.data_utils.prep_optitrack_for_rt import convert_optitrack_pkl_to_npz
...
def test_convert_optitrack_pkl_to_npz_outputs_standard_rt_npz(tmp_path: Path) -> None:
    ...
    for t in range(2):
        ...
        for j, joint_name in enumerate(OPTITRACK_DEMO_JOINTS_27):
            frame[joint_name] = [
    ...
    assert joints.shape == (2, len(OPTITRACK_DEMO_JOINTS_27), 3)
